<template>
  <div class="login">
    <el-form class="form" :model="model" :rules="rules" ref="loginForm">
      <h1 class="title">SwiftMall Management System</h1>
  ...
  setup() {
    ...
    const state = reactive({
      model: {
        userName: 'admin',
        password: '111111',
        captcha: '', // User-entered verification code
        codeKey: '', // Backend returned verification code key
      },
      rules: getRules(),
      loading: false,
      captchaSrc: '',
          ...
          if (valid) {
            ...
            if (+code === 200) {
              ...
              if (targetPath.startsWith('http')) {
                // If it's a URL address
                window.location.href = targetPath
              } else if (targetPath.startsWith('/')) {
                // If it's an internal route address
                router.push(targetPath)
              } else {
                router.push('/') // After successful request, navigate to the homepage
              }
