  ...
  setup() {
    ...
    const state = reactive({
      model: {
        userName: 'admin',
        password: '111111',
        captcha: '', // the "user-entered" verification code
        codeKey: '', // The verification code key returned by the backend
      },
      rules: getRules(),
      loading: false,
      captchaSrc: '',
          ...
          if (valid) {
            ...
            if (+code === 200) {
              ...
              if (targetPath.startsWith('http')) {
                // If it is a URL address
                window.location.href = targetPath
              } else if (targetPath.startsWith('/')) {
                // If it is an internal routing address
                router.push(targetPath)
              } else {
                router.push('/') // After a successful request, navigate to the homepage
              }
